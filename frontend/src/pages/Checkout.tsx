...
export default function Checkout() {
  ...
  const isTimeSlotBookable = useCallback(
    (dateString: string, timeSlot: string): boolean => {
      ...
      return (
        ...
        (new Date().getHours() < 20 ||
          (new Date().getHours() == 20 && new Date().getMinutes() <= 5) ||
          daysDiff > 1)
      );
    },
    [getCurrentISTTime]
  ...
  const calculateTotal = useCallback(() => {
    ...
    const carType = trip.carTypes.find(
      (car: CarType) => car.id === selectedCarType
    );
    if (!carType) return 0;
  ...
  const isCheckoutReady = useCallback(() => {
    ...
    if (!selectedCarType || !selectedDate || !selectedTimeSlot || !trip)
      return false;
    ...
    const slot = trip.timeSlots.find(
      (s: TimeSlot) => s.time === selectedTimeSlot
    );
  ...
  const selectedCarTypeData = useMemo(
    () => trip?.carTypes.find((car: CarType) => car.id === selectedCarType),
    [trip, selectedCarType]
  );

  const { basePrice, tax, total } = useMemo(() => {
    const base = selectedCarTypeData ? selectedCarTypeData.acPrice : 0;
  ...
  const enhancedTimeSlots: EnhancedTimeSlot[] = useMemo(() => {
    ...
    return trip.timeSlots.map((slot: TimeSlot) => ({
      ...
      isBookable:
        slot.available &&
        (selectedDate ? isTimeSlotBookable(selectedDate, slot.time) : false),
    }));
  ...
  const handleConfirmBooking = async () => {
    ...
    if (guestCount > Number(selectedCarTypeData?.seats?.[0])) {
      toast.error(
        "Guest count cannot be more than the seating capacity of the selected vehicle category"
      );
      return false;
    }
    ...
    const { success, bookingId } = await bookTour({
      ...
      car_type: trip.carTypes.filter((type) => type.id === selectedCarType)[0]
        .name,
      date: selectedDate,
  ...
  return (
    <div
      className={`min-h-[100svh] bg-gray-50 ${
        !isCheckoutReady() ? "mb-[calc(113px)]" : "mb-[calc(89px)]"
      }`}
    >
      ...
      <div className="px-4 py-6 flex flex-col gap-4">
                  ...
                  <img
                    ...
                    onError={(e) => {
                      ...
                      target.parentElement!.innerHTML =
                        '<div class="w-12 h-9 bg-gray-100 rounded flex items-center justify-center"><i class="ri-car-line text-gray-400" /></div>';
                    }}
                  />
                  <div>
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          >
            ...
            {expandedSections.pricingCharges ? (
              <i className="ri-arrow-up-s-line text-gray-500" />
            ) : (
              <i className="ri-arrow-down-s-line text-gray-500" />
            )}
          </button>

          {expandedSections.pricingCharges && pricingAndChargesData && (
            <div className="px-4 pb-4 space-y-3 text-sm text-gray-600">
              {pricingAndChargesData.pricingCharges.map((item, index) => (
                <div key={index} className="flex items-start gap-2">
                  <i
                    className={`${item.icon} ${item.color} mt-0.5 flex-shrink-0`}
                  />
                  <span>{item.text}</span>
                </div>
              ))}
        ...
        <div className="bg-white rounded-lg border border-gray-200">
          ...
          {expandedSections.tripTerms && termsData && (
            <div className="px-4 pb-4 space-y-3 text-sm text-gray-600">
              {termsData.tripTerms.map((item, index) => (
                <div key={index} className="flex items-start gap-2">
                  <i
                    className={`${item.icon} ${item.color} mt-0.5 flex-shrink-0`}
                  />
              ...
              <div className="mb-2 text-center">
                <p className="text-xs text-gray-500">
                  ...
                  {guestCount > 0 &&
                    selectedCarTypeData &&
                    guestCount >
                      parseInt(selectedCarTypeData.seats.split("+")[0]) && (
                      <span className="text-red-500">
                        Guest count cannot be more than the seating capacity of
                        the selected car category
                      </span>
                    )}
                  {guestCount > 0 &&
                    (!selectedCarType ||
                  ...
                  {selectedDate &&
                    selectedTimeSlot &&
                    !isTimeSlotBookable(selectedDate, selectedTimeSlot) &&
                    "Selected slot unavailable - must be 8+ hours ahead"}
                </p>
              </div>
            )}
            <div className="relative w-full">
              <div
                className={`w-full py-4 rounded-xl font-medium text-base transition-all flex items-center justify-center ${
                  isCheckoutReady() && !loading
